...
@router.callback_query(StateFilter(FSMFillForm.choose_department),
                       F.data.in_(['b2b']))
async def b2b_sku_group_choose(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        text='Укажите группу артикула, с которым произошла проблема:\n\n',
        reply_markup=markup
    )
...
async def process_order_number(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f'Выбран артикул: {lexicon_dict[info["b2b_sku_group"]]}{info["sku_number"]}\n\n'
             f'Укажите номер заказа:'
...
async def process_order_number(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f'Выбран артикул: {lexicon_dict[info["b2b_sku_group"]]}{info["sku_number"]}\n'
             f'Номер заказа: {info["order_number"]}\n\n',
...
async def process_b2b_worker(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        text=f'Выбран артикул: {lexicon_dict[info["b2b_sku_group"]]}{info["sku_number"]}\n'
             f'Номер заказа: {info["order_number"]}\n'
             f'Выбрана дата: {info["date"]}\n'
             f'Выбран тип проблемы: {lexicon_dict[info["b2b_type_of_problem"]]}\n\n'
             f'Укажите ответственного за сборку:',
    )
...
async def process_b2b_comment(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f'Выбран артикул: {lexicon_dict[info["b2b_sku_group"]]}{info["sku_number"]}\n'
             f'Номер заказа: {info["order_number"]}\n'
             f'Выбрана дата: {info["date"]}\n'
             f'Выбран тип проблемы: {info["b2b_type_of_problem"]}\n'
...
async def process_date_sent(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f'Выбран артикул: {lexicon_dict[info["b2b_sku_group"]]}{info["sku_number"]}\n'
             ...
             f'Выбран тип проблемы: {info["b2b_type_of_problem"]}\n'
             f'Ответственный за сборку: {info["b2b_worker"]}\n'
             f'Комментарий к проблеме: {info["b2b_comment"]}\n\n',
...
@router.callback_query(StateFilter(FSMFillForm.b2b_choose_date),
                       F.data.in_(['calendar']))
async def process_b2b_date(callback: CallbackQuery):
    await callback.message.answer(
        "Укажите дату:",
        reply_markup=await SimpleCalendar().start_calendar()
    )
